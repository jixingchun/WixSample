namespace CustomBA
{
    ...
    using Bootstrapper = WixToolset.Bootstrapper;
    ...
    public class InstallationViewModel : PropertyNotifyBase
    {
            ...
            get
            {
                if (this.completeCommand == null)
                {
                    this.completeCommand = new RelayCommand(param =>
                    {
                        ...
                        if (this.root.InstallState == InstallationState.Applied)
                        {
                            switch (this.root.DetectState)
                            {
                                ...
                                case DetectionState.Absent: // Î´°²×°
                                    {
                                        WixBA.View.Hide();

                                        WixBA.Model.Engine.Log(LogLevel.Verbose, $"this.root.InstallState ={ this.root.InstallState.ToString()}");
                                        InstallCompleteViewModel viewModel = new InstallCompleteViewModel();
                                        InstallCompleteView view = new InstallCompleteView(viewModel);
                                        view.ShowDialog();
                                        WixBA.View.Close();
                                        break;
                                    }
                        ...
                        else if (this.root.InstallState == InstallationState.Failed)
                        {
        ...
        public ICommand InstallCommand
        {
            get
            {
                if (this.installCommand == null)
                {
                    ...
                    {
                        WixBA.Plan(LaunchAction.Install);
                    }
                    ,
                    param => this.root.DetectState == DetectionState.Absent 
        ...
        {
            WixBA.Model.Engine.Log(LogLevel.Verbose, $"DetectBegin: Installed= {e.Installed}, PackageCount ={e.PackageCount}, Result = {e.HResult}");
        ...
        private void ExecutePackageComplete(object sender, ExecutePackageCompleteEventArgs e)
        {
            ...
            if (DateTime.MinValue < this.executePackageStart)
            {
                this.AddPackageTelemetry("Execute",
                    e.PackageId ?? String.Empty, DateTime.Now.Subtract(this.executePackageStart).TotalMilliseconds, e.Status);
